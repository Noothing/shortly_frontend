<script>
     ...
     * Get context
     */
    const {updateGood} = getContext("goods")
    ...
    const validateAll = async () => {
        ...
        for (let i = 0; i < form.length; i++) {
            ...
            if (field.value != good[field.name]) {
        ...
        if (errored === 0) {
            const res = await updateGood(good.id, formData)
    ...
    const initAll = () => {
        for (let i = 0; i < Object.keys(good).length; i++) {
            const key = Object.keys(good)[i]
            const field = form.find(e => e.name == key)
            if (field) {
    ...
    <div class="create">
        ...
        <div class="create__image" on:click={openInput}>
            ...
            {#if form[0].value != null}
                {#if form[0].value.search('base64') === -1}
                    <img src='{baseURL}/uploads/{form[0].value}'/>
                {:else}
